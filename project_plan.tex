...
We will build a \textbf{pursuit demo} in \textbf{Godot}: a pursued actor
(either player controlled or with simple flee logic) moves through a scene
containing static and dynamic obstacles. Two to three AI controlled
pursuers navigate in parallel using different path planning algorithms
toward the same target. During the simulation, benchmarks are recorded
live and displayed as a comparison chart at the end.


\subsection{Game Concept in Detail}

\begin{itemize}[leftmargin=1.2em]
  \item \textbf{Scenario:} top down view, a grid or navigation structure
  with obstacles (walls, rooms, narrow passages), similar to the example
  scenes from the lecture (waypoints, grid, polygon meshes).
  \item \textbf{Mode 1, Spectator Race:} multiple agents start at the same
  point and pursue the same moving target. Whoever arrives first, or
  follows the shortest, fastest route, wins visibly.
  \item \textbf{Mode 2, Live Benchmark:} on button press, a course is
  solved by several algorithms simultaneously, without a player character,
  purely for data collection.
  \item \textbf{Dynamic obstacles:} some obstacles move on
  their own so that replanning becomes necessary. This makes the
