extends Node2D
## Wires the Grids, NPCs, and Obstacles together. Walls come entirely from
## the maze generator, so left click no longer edits the map, it alternates
## between placing every active NPC's spawn point and sending them all to a
## shared goal cell. Right click restarts the whole game at any time,
...
## algorithm's own search visible in isolation and removes any chance of
## NPCs stepping on each other's occupied cells.
##
## Obstacles are not a single fixed scene node: StartUI asks the player how
## many to spawn (per grid) before the run begins, so they get instantiated
## from OBSTACLE_SCENE at runtime and parented under that grid's own
## ObstacleContainer. NPCs work the same way, one per algorithm the player
## ticked on StartUI, instantiated from NPC_SCENE and parented under its
## own grid's NPCContainer. They never collide with each other, only
## against their own grid's Obstacles.
...
const LIFE_DASHBOARD_SCENE: PackedScene = preload("res://scenes/LifeDashboard.tscn")
...
# several grids stay tellable apart at a glance.
...
# of tinting a single shared sheet.
...
const SIDEBAR_WIDTH: int = 460 # px, StartUI's own width, see START_WINDOW_SIZE
...
const DASHBOARD_HEIGHT: int = 320 # px, extra window height reserved below the grids for results
# Window size before Start is pressed: just StartUI, no maze/grid behind it.
const START_WINDOW_SIZE: Vector2i = Vector2i(SIDEBAR_WIDTH, 760)
...
# Filled in on a goal click, read back out once every entry has stopped
# "running". Entry shape: {"status": "running"/"done"/"failed", "start_ms":
# int, "explored": int, "path_length": int, "elapsed_ms": int}. Only
# "status" and, for "running"/"done", "start_ms" are guaranteed present at
# every stage, see _on_cell_clicked()/_on_npc_arrived().
var _benchmarks: Dictionary = {}
var _dashboard: BenchmarkDashboard = null # currently shown results panel, if any

# Life mode's live benchmark tracking, one entry per NPC, keyed by the NPC
# itself. Unlike _benchmarks there is no "status", every entry just keeps
# growing: {"start_ms": int, "reached_count": int, "total_explored": int,
# "total_path_length": int}, the latter two summed across every completed
# leg so far, so their ratio is a stable cumulative efficiency rather than
# one that jumps around leg to leg.
var _life_benchmarks: Dictionary = {}
var _life_dashboard: LifeDashboard = null # currently shown live panel, if any

# True once a spawn point has been placed and we are waiting for the
# matching left click that sets the goal. False means the next left click
# places a new spawn point instead.
...
## player picks Race/Life, obstacle count, and algorithms first, without
## any maze or NPCs showing behind the panel. Nothing about the actual run
## is generated until Start is pressed, see _on_start_pressed(). Right
## click restart (see _unhandled_input()) still works from the first frame.
##
## reload_current_scene() only reloads the scene tree, not the OS window
## itself, so a previous run's maximized state and large content_scale_size
## (set by _configure_window() once Start was pressed) would otherwise
## still be in effect here. Every window property _configure_window() can
## change has to be reset back to a plain, unscaled window here too.
func _ready() -> void:
	grid.visible = false
	...
	window.size = START_WINDOW_SIZE

	start_ui.start_pressed.connect(_on_start_pressed)
	grid.cell_clicked.connect(_on_cell_clicked)

## Only now, once the player has pressed Start, does the run itself begin:
## generates the maze, reveals the grid, then builds one Grid per selected
## algorithm (all sharing that identical maze layout), plans
...
func _on_start_pressed(obstacle_count: int, algorithms: Array[String], mode: String) -> void:
	...
	for i in algorithms.size():
		...
		npc.life_spot_reached.connect(_on_npc_life_spot_reached.bind(npc))
...
## dedicated grid to compare algorithms on without competing for the same
## cells. Reuses the original preview Grid (already holding the maze from
## _ready()) as the first one instead of throwing it away. Falls back to
## that single grid if count is 0 (no algorithm selected).
...
## planning pass is reused to instantiate matching Obstacles into every
## grid further down (see _instantiate_obstacles()), meaning every
## comparison grid sees the exact same obstacle behaviour. Planning has to
## happen before any spawning: start_path() immediately blocks an
## Obstacle's own spawn cell, and is_walkable() checks that same occupancy,
## so planning route 2 after already spawning Obstacle 1 could reject
## route 2 if it started or ended on Obstacle 1's spawn cell.
...
## ObstacleContainer, so occupancy tracking (grid.set_cell_occupied())
## stays fully independent per grid even though the routes themselves are
## shared, keeping every grid's patrol behaviour in lockstep.
...
## leftover NPC/Obstacle/Grid/dashboard state, back to StartUI. Handled
## here globally (Main receives every unhandled input, unscoped to any
## position) rather than per Grid instance: a Grid only ever emits
## cell_clicked for a click inside its own columns x rows area, so it used
## to miss a right click anywhere else on screen, e.g. once the benchmark
## dashboard grew the window and added space below the grids.
...
## the same cell coordinate is valid in all of them at once, so one click
## really does set every grid's spawn point together. In Race mode, the
## next left click after that sets the shared goal. In Life mode there is
## no second click: as soon as the spawn point is placed, every NPC heads
## for spot A on its own, then keeps cycling A -> B -> C -> A ... without
## further input. Every npc.go_to() call (here and, in Life mode, every
## later leg triggered by LifeWaitState) refreshes its own grid's overlay
## entry on its own, see npc.gd's _update_grid_overlay(), so nothing here
## needs to touch any Grid's overlay beyond clearing stale entries below.
func _on_cell_clicked(cell: Vector2i) -> void:
	...
	if not _awaiting_goal_click:
		...
		if is_instance_valid(_life_dashboard):
			_life_dashboard.queue_free() # from a previous Life mode run, same reason
		for npc in _npcs:
			npc.place_at(cell)
		...
		if _life_mode:
			...
			_life_benchmarks.clear()
			for npc in _npcs:
				_life_benchmarks[npc] = {
					"start_ms": start_ms, "reached_count": 0, "total_explored": 0, "total_path_length": 0
				}
				npc.go_to(npc.life_spots[0]) # spot 1, LifeWaitState takes it from there
			_show_life_dashboard()
		else:
...
## search already ran synchronously inside go_to(), so explored cells and
## path length are already known right away, only the elapsed walking time
## still needs _on_npc_arrived() later. A goal pathfinding could not reach
## at all leaves current_path() empty, marked "failed" right here instead
## of waiting for an arrival that will never come.
...
## one from an earlier race. Grows the window first
## (_reserve_dashboard_space()) so the panel gets its own strip below the
## grids instead of covering them.
...
## Fired by every NPC's own life_spot_reached signal, which (unlike
## arrived) only ever fires on a genuine hotspot arrival, see npc.gd's own
## doc comment on that signal. Adds this leg's explored cells and path
## length to the running totals, then refreshes that NPC's row in the live
## dashboard. The rate itself (hotspots per minute) is computed
## continuously inside LifeDashboard's own _process(), not here, since it
## depends on elapsed time as much as on reached_count.
func _on_npc_life_spot_reached(npc: NPC) -> void:
	if not _life_benchmarks.has(npc):
		return
	var entry: Dictionary = _life_benchmarks[npc]
	entry["reached_count"] += 1
	entry["total_explored"] += npc.pathfinder.explored_cells.size()
	entry["total_path_length"] += npc.current_path().size()
	_life_benchmarks[npc] = entry

	if is_instance_valid(_life_dashboard):
		_life_dashboard.update_entry(_npcs.find(npc), entry["reached_count"])

## Replaces any still-open live panel from an earlier Life mode run with a
## fresh one, then hands it one starting row per NPC (see
## _build_life_entries()). Reserves the same bottom strip the Race mode
## dashboard uses (_reserve_dashboard_space()), just filled in continuously
## instead of only once at the end.
func _show_life_dashboard() -> void:
	if is_instance_valid(_life_dashboard):
		_life_dashboard.queue_free()
	_reserve_dashboard_space()
	_life_dashboard = LIFE_DASHBOARD_SCENE.instantiate()
	add_child(_life_dashboard)
	_life_dashboard.show_entries(_build_life_entries())

## One starting entry per NPC: label/color for display, plus the shared
## start_ms every NPC's _life_benchmarks entry was just given, so
## LifeDashboard can tick its own elapsed-time display independently.
func _build_life_entries() -> Array[Dictionary]:
	var entries: Array[Dictionary] = []
	for npc in _npcs:
		var algorithm_id: String = _npc_algorithm_ids.get(npc, "")
		var benchmark: Dictionary = _life_benchmarks.get(npc, {})
		entries.append({
			"label": ALGORITHM_LABELS.get(algorithm_id, algorithm_id),
			"color": ALGORITHM_COLORS.get(algorithm_id, Color.WHITE),
			"start_ms": benchmark.get("start_ms", Time.get_ticks_msec()),
		})
	return entries
...
## StartUI has already queue_freed itself, so there is no sidebar to
...
## Tells Godot's own window content scaling to stretch content_size worth
## of logical game pixels to fill the actual window, then maximizes that
## window, always ending up as large as the current screen allows.
##
## content_scale_size + MODE_MAXIMIZED is the standard resolution
## independent Godot approach: Godot's own rendering pipeline does the
## stretching every frame, and maximizing is a plain OS level window
## state, neither depends on any value left over from a previous run
## (a manual DisplayServer.screen_get_usable_rect() + raw pixel resize
## approach was tried first and found unreliable across editor restarts
## and window moves).
...
## Re-tells Godot's window content scaling to include DASHBOARD_HEIGHT
## worth of extra logical height below the grids, so the results panel
## gets its own space instead of covering them. Purely a content_scale_size
## change (not a raw pixel window resize), recomputed fresh from
