...
## based is_walkable() check, which still does the smart, proactive rerouting.
##
...
## Fired every time DoneState is entered: on spawn placement, on an
## immediately unreachable goal, and on a genuine Race mode arrival alike.
## main.gd's benchmark tracking tells those apart itself (it only treats
## this as "arrived" while actively timing this NPC).
signal arrived

## Fired every time LifeWaitState is entered, i.e. every time this NPC
## reaches one of its life_spots (Life mode only). Unlike arrived, this
## always means a genuine arrival, so main.gd's live benchmark tracking can
## treat every firing as "one more hotspot reached."
signal life_spot_reached
...
# several NPCs can be told apart without a shared sheet plus modulate tint.
# Must be set (see main.gd) before this NPC enters the tree, _ready() below
# reads it right away.
...
## BlockState can recompute a fresh path from wherever the NPC currently is
## if the route gets blocked mid walk (only relevant for pathfinders whose
## block_reaction() is REPLAN, RetreatState never calls this).
##
## An empty result does not mean the goal is unreachable for good: with
## enough patrolling Obstacles a route can be blocked for just one instant.
## Handing that off to BlockState (instead of giving up in DoneState) lets
## the NPC keep retrying on its own until a route opens up.
func go_to(goal: Vector2i) -> void:
	_goal_cell = goal
	_set_path(pathfinder.find_path(grid, current_cell, goal))
	_update_grid_overlay()
	fsm.change_state("BlockState" if _path.is_empty() else "WalkState", {"facing": _facing})
...
## Only replaces the path data when a route is actually found, leaving the
## still-blocked path untouched on failure (an empty path would otherwise
## satisfy path_finished() and get mistaken for "arrived"). The caller
## (BlockState) decides what FSM state to move to based on the result.
...
## Keeps Grid's debug overlay showing this NPC's actual current path/explored
## cells. Called every time a fresh path is computed, so Life mode's
## ongoing cycle and mid walk replans both stay reflected too.
func _update_grid_overlay() -> void:
	grid.set_search_result(self, pathfinder.explored_cells, _path, accent_color)

## Teleports the NPC to cell right away and cancels any path in progress,
## used to set a new spawn point by clicking the grid.
...
## obstacle. Called by WalkState from _physics_process.
...
## True if this physics step's _check_obstacle_collisions() found an overlap.
...
## without moving once path_index is already 0.
...
## CollisionSystem's hand written AABB-Circle check. Pushes the NPC back
## out of any obstacle it now overlaps.
...
## Works out how many columns/rows of frames the sheet has, from the pixel
