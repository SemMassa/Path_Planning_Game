...
## blocked map. Produces a plain walkable grid in the same [x][y] layout
## Grid uses internally, load it via Grid.load_walkable_map().
...
## Builds a maze of the given size and returns it as walkable data, ready
## for Grid.load_walkable_map(). Call room_center() / room_count()
...
## an existing room (with margin). Does nothing on failure, the caller
## just retries with the next attempt.
...
func _connect_rooms(a: Rect2i, b: Rect2i) -> void:
	...
	# Either L shape (horizontal first or vertical first) connects the same
	# two rooms equally well, pick whichever runs beside fewer already
	# carved corridor cells, keeping corridors from ending up parallel and
	# adjacent, which would read as one wide corridor.
...
## All cells of one L shaped route between two points, either horizontal
## first (sideways, then up/down) or vertical first.
func _l_path_cells(
