...
extends RefCounted
## Hand written broad phase and narrow phase collision test between the
## NPC's circle and an Obstacle's box. Godot's physics engine is not used
## for this on purpose: the assignment asks for the detection phases
## themselves, not an engine call.

## Broad phase: cheap axis aligned bounding box overlap test. Filters out
## pairs that are nowhere near each other before the exact circle test below.
...
## returns center itself, with no direction to escape in.
