class_name Obstacle
extends Node2D
## Patrols back and forth along a fixed list of cells (ping pong). Blocks
## the cell it is currently in and frees the one it just left, exactly
## when its visible position crosses into a new cell, so the map always
## matches what is on screen. Must be parented under a Grid node, same as NPC.
##
## Drawn as its own filled, outlined square (see _draw() below) rather than
## only showing up as a blocked grid cell: an occupied cell used to be
## painted the same black as a real wall, which made the Obstacle look
## like it was merging into and walking through walls whenever it passed one.
##
## Plain Node2D, not a physics body: nothing here goes through Godot's
## physics server, box_size below is just plain exported data for NPC's
## hand written collision check (see collision_system.gd) to read. This
## also sidesteps a physics body's own position/transform syncing, which
## could delay a freshly spawned Obstacle's first visible position by a
## frame, showing it briefly at its default (0, 0) before jumping to its
## real spawn cell.

const FILL_COLOR: Color = Color(0.754, 0.137, 0.842)
...
## picks it, e.g. a room center). Placement does not depend on path being
## usable: if pathfinding fails and returns an empty (or single cell)
## route, the Obstacle still ends up sitting at a real floor cell.
...
## letting it walk straight through.
...
## Draws the obstacle as a filled, outlined square matching box_size, so
## it stays visible as a distinct shape at its real, continuous position,
