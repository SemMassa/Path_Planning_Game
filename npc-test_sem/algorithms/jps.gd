...
##
## Restructured from a cardinal-only JPS+ reference (right/left/up/down
## only, "matches the NPC sprites") into full 8-directional JPS with
## diagonal jump points, at the user's explicit request, so path lengths
## stay directly comparable with A*/D* Lite. Two further adaptations, same
## reasoning as dstar_lite.gd: a single find_path(grid, start, goal) entry
## point (not compute_path(Vector2,...)), and Array[Vector2i] cells instead
## of a PackedVector2Array of world positions.
##
...
## (proven while porting this: the blocked cell the classic rule tests is
## always one of the two flanking cells that exact diagonal move would need
## open, so it can never actually fire here). _cardinal_extra_directions()
## below is the strict-corner-cutting replacement: a pure perpendicular
## escape when the matching diagonal would be illegal, plus a genuine
## diagonal escape only when that diagonal only just became legal at this
## cell (not one step further back), so a diagonal never gets proposed
## unless it is actually walkable. Verified against a from-scratch Dijkstra
## reference across ~1500 randomized mazes/positions before this port.
##
## The start cell always counts as open even if an Obstacle currently
## occupies it (kept from the reference): NPC's own current_cell briefly
## overlapping a moving Obstacle should not make replanning from there
## impossible.
...
func find_path(grid: Grid, start: Vector2i, goal: Vector2i) -> Array[Vector2i]:
	...
	while not open_list.is_empty():
		var current: Vector2i = _pop_lowest_f(open_list, g_score, goal)
		...
		for direction in _pruned_directions(grid, current, came_from):
			...
			var tentative_g: float = g_score[current] + _heuristic(current, jump_cell)
...
## Removes and returns the open cell with the lowest f_score (g_score plus
## heuristic), linear scan like AStarPathfinder's _pop_lowest_f_score().
func _pop_lowest_f(open_list: Array[Vector2i], g_score: Dictionary, goal: Vector2i) -> Vector2i:
	var best_index: int = 0
	var best_f: float = g_score[open_list[0]] + _heuristic(open_list[0], goal)
	for i in range(1, open_list.size()):
		var f: float = g_score[open_list[i]] + _heuristic(open_list[i], goal)
		if f < best_f:
			best_f = f
			best_index = i
	return open_list.pop_at(best_index)

## Octile distance: the true cost between two cells that are guaranteed
## collinear along one of the 8 directions (as any two consecutive jump
## points always are), and also this search's admissible heuristic to the
## goal, exactly like AStarPathfinder/DStarLitePathfinder use it.
