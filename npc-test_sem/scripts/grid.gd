...
# itself (Objects hash by identity, so this just replaces one NPC's own
# entry each time it gets a new path, never mixes NPCs up). Each entry is
# {"explored": Dictionary (Vector2i -> true, unordered set), "path":
# Array[Vector2i], "color": Color}, color matches that NPC's own sprite
# tint (see main.gd's ALGORITHM_COLORS) so several overlays stay tellable
# apart. _overlay_enabled is the runtime on/off toggle (key O), separate
# from the data itself, so toggling off and back on does not lose anything.
...
# Each Grid instance now owns its own Obstacles/NPCs (main.gd builds one
# Grid per active NPC so they can be compared side by side without ever
# sharing cells), instead of a single pair of containers shared by all of
# them under Main.
...
## order. Used to draw random spawn/patrol cells for Obstacles from. Plan
## every Obstacle's route with this (and is_walkable(), unaffected by
## occupancy since none exists yet) before spawning any of them, an
## Obstacle already spawned and occupying its own start cell would
## otherwise make is_walkable() reject the very next Obstacle's route.
...
## Removes every NPC's search-result overlay, used whenever the map changes
## and none of the previous search results reflect reality anymore (a new
## shared spawn point was just placed).
func clear_search_results() -> void:
	_overlays.clear()
	queue_redraw()

## Flips whether the overlay (every NPC's explored cells + path, if any)
## is currently drawn at all. The underlying data is untouched, so toggling
## back on immediately shows whatever was last set.
...
## NPC's color, so several side by side grids stay tellable apart at a
## glance. Pass an empty string to hide it again.
func set_title(text: String, color: Color) -> void:
	_title = text
	_title_color = color
	queue_redraw()

## Marks cell as the current shared spawn point (drawn as a green marker
## labeled "A"). Pass NO_CELL to hide it again.
func set_start_cell(cell: Vector2i) -> void:
	_start_cell = cell
	queue_redraw()

## Marks cell as the current Race mode goal (drawn as a red marker labeled
## "B"). Pass NO_CELL to hide it again, e.g. once a new spawn point is
## placed and no goal has been chosen for it yet.
...
## walkable/wall. A cell an Obstacle currently stands on is NOT tinted
## separately here, Obstacle draws its own square now (see obstacle.gd), so
## the cell underneath it can just stay plain floor.
...
## so several overlays stay tellable apart.
...
## mode spot, so it is obvious on the grid where the NPCs are actually
## headed, not just visible once they arrive there.
...
## Centers `label` roughly on `center`. draw_string() positions by baseline,
## not a bounding box, so this is an approximation, plenty good enough for
## a single short letter on a small marker.
...
## "A") and/or the goal cell (red, "B"), same look as the Life mode spot
## markers, so the current spawn/goal stay identifiable at a glance.
...
## Paints every cell using _cell_color(), a thin grid outline, then the
## start/goal markers, every NPC's path, the Life mode spot markers, and
## this grid's own title on top.
func _draw() -> void:
	for x in columns:
		for y in rows:
			...
			#draw_rect(rect, Color(0.842, 0.436, 0.713, 1.0), false)
	_draw_start_goal_markers()
	_draw_paths()
	_draw_life_spots()
	_draw_title()

## Translates a left click into a grid cell and forwards it as a signal, and
## the O key into the overlay on/off toggle, works at any time, before or
## after Start. Right click (restart) is handled globally by main.gd
## instead, not scoped to any one grid's own cell area, see its own
## _unhandled_input() doc comment for why.
