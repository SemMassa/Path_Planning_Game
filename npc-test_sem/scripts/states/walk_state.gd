...
## walkable (the patrolling Obstacle moved onto it), or a real physical
## collision happens anyway (a safety net alongside that proactive grid
## check), hands off to whichever state matches the current pathfinder's
## block_reaction(): RetreatState for algorithms meant to stick to their
## original path (plain A*), BlockState for ones meant to replan.
...
## driving this NPC, not hardcoded, so plain A* and a future replanning
## algorithm (e.g. D* Lite) can react completely differently to the exact
## same blockage.
