...
## RETREAT (plain A*): never recomputes a route. Instead steps backward
## through the cells already walked, as if physically pushed back by
## whatever is blocking the next forward cell, until that cell is
...
## one cell closer with every retreat_step(), so it would start pointing
## at a cell behind us that was never blocked in the first place, making
## this think the way is clear after a single step back.
